...
% Target length: 8-10 pages including figures, excluding bibliography.
% =====================================================================
...
question, 15 per mode) and \emph{no cache}; synthesis is excluded. Commands:
\cmd{uv run python benchmarks/benchmark\_sources.py -{}-repeats 5} (offline) and
\cmd{uv run python benchmarks/benchmark\_live\_sources.py -{}-repeats 1} (live);
the full reproducer is in the appendix.
...
\midrule
\cmd{LLM\_PROVIDER} (\cmd{gemini} in \cmd{.env.example}) & The supplied factory falls back to \cmd{anthropic}; without its key or SDK, synthesis fails and evidence is printed (exit 1). \\
...
The remaining variables all fall back to the default in brackets when missing:
\cmd{LOG\_LEVEL} (INFO), \cmd{MAX\_QUESTION\_LENGTH} (2000),
\cmd{PER\_SOURCE\_TIMEOUT\_SECONDS} (10), \cmd{MAX\_SOURCES\_PER\_QUERY} (3),
\cmd{RETRY\_MAX\_ATTEMPTS} (3), \cmd{RETRY\_INITIAL\_DELAY\_SECONDS} (0.5),
\cmd{RETRY\_MAX\_DELAY\_SECONDS} (4) and \cmd{CACHE\_TTL\_SECONDS} (86400).
...
\textbf{AI assistance.} The table repeats the signed contribution statement, plus
one row for this revision. We can defend every line of code; ``the AI wrote it''
is not an answer we use.

\begin{center}\footnotesize
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.34\textwidth}>{\raggedright\arraybackslash}p{0.13\textwidth}X@{}}
\toprule
\textbf{Module / file} & \textbf{Assistant} & \textbf{What we did with it} \\
\midrule
\cmd{docs/figures/architecture.*}, architecture and cache documents,
\cmd{tests/cache\_verification.py}, \cmd{tests/test\_cache\_*.py}; first drafts of
this report and the deck & Claude (Anthropic) &
Drafted the figure, harness, cache tests and documents. Samur Eyyubov reviewed
each file, ran the suite and harness, and corrected facts against the code. \\
Researcher application code, tests, CI/Docker setup, documentation & OpenAI Codex &
Implementation drafts, debugging, test design, CI/Docker configuration and
documentation. The responsible members reviewed and adapted the output and
verified the behaviour before committing. \\
\cmd{report/report.tex}, \cmd{slides/slides.tex} (final revision) & Claude (Claude Code) &
Aligned both documents with the templates: refreshed figures, added this
table, rebuilt the slides. Reviewed in a pull request. \\
\bottomrule
\end{tabularx}
\end{center}
